...
- Базовая версия для курса: Python 3.14.x; Python 3.15 pre-release используйте только для обзора будущих изменений.
- Для новых проектов используйте `pyproject.toml`, `uv` или `venv + pip`, `ruff`, `pytest`, `mypy`/`pyright`, `pre-commit`.
- В уроках явно разделяйте учебный Python, production Python, data science notebooks и automation scripts.
- В интерактивном курсе делайте автопроверку через pytest, проверку типов, форматирование и скрытые кейсы ввода/вывода.
...
## Тема 3. Переменные, ввод/вывод, комментарии, форматирование

Коротко:
- Переменные создаются при присваивании: `x = 10`. Типы динамические, но можно добавлять аннотации: `x: int = 10`.
- Ввод/вывод: `input()`, `print()`. Для форматирования — f‑строки.
- Комментарии: `#` для строки, `"""docstring"""` для документации.
...
Строки неизменяемы; методы: `split`, `join`, `strip`, `replace`, `find`. Форматирование: f‑строки, `str.format`.
```python
s = "a,b, c "
parts = [p.strip() for p in s.split(",")]
t = "-".join(parts)  # 'a-b-c'
name = "Ann"
print(f"{name=}")    # debug‑формат
```
...
Режимы: 'r', 'w', 'a', 'b', менеджер контекста `with`. Для путей используйте `pathlib.Path`.
```python
from dataclasses import dataclass
from pathlib import Path
@dataclass
class User: name:str; age:int
# современный вариант через pathlib
...
Структура: `src/`, `tests/`, `pyproject.toml`, `README.md`. Зависимости: `pip`, `uv`, `poetry`, `pdm`, `pip-tools`.

## Тема 60. Настройка среды и параметров.

Интерпретатор/venv, форматтер (black), линтер (ruff), типизация (mypy/pyright), тесты (pytest).
...
## Тема 86. Современное управление зависимостями и упаковка

Коротко:
- `pyproject.toml` — единая точка конфигурации проекта; для чтения TOML без внешних зависимостей используйте `tomllib` (stdlib, Python 3.11+).
- Сборка: `setuptools`, `hatchling`, `poetry-core` (выбирайте под проект).
- Утилиты: `pipx` для установки CLI, `uv`/`pip-tools` для lock‑файлов.

Мини‑пример (фрагмент `pyproject.toml`):
```toml
[project]
name = "demo"
version = "0.1.0"
...
## Тема 87. Типизация и качество кода

Коротко:
- Типизация через `typing`; в Python 3.14 аннотации откладываются по умолчанию, а `from __future__ import annotations` нужен в первую очередь для совместимости со старыми версиями.
- Анализаторы: `mypy` или `pyright`.
- Форматтер/линтер: `black`, `ruff`, хуки `pre-commit`.
...
## Тема 88. Асинхронность и конкурентность

Коротко:
- `asyncio` для конкурентного I/O.
- `async/await`, `TaskGroup` для структурной конкурентности, отмена через `asyncio.timeout`.
- Для изоляции вычислений и параллельной работы с интерпретаторами в Python 3.14 появился модуль `concurrent.interpreters`.
- Библиотеки: `httpx`, `aiofiles`, `anyio`.
...
## Тема 89. Заготовка для интерактивного курса

Коротко:
- Модули: `py-basics`, `py-data-structures`, `py-functions`, `py-oop`, `py-files`, `py-testing`, `py-async`, `py-packaging`.
- Автопроверка: `pytest`, property-based тесты через `hypothesis`, проверка stdout, проверка `ruff`, проверка типов `pyright` или `mypy`.
- Проектная линия: CLI заметки -> хранение в JSON/SQLite -> классы и dataclasses -> тесты -> упаковка -> простой API.
- Платные элементы: разбор ошибок, ревью итогового проекта, индивидуальные задания на automation/data/backend.
