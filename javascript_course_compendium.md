...
### 2.1 Вузовская сеть и npm
В учебных аудиториях возможны прокси, подмена TLS сертификатов и частично закрытые registry. Для лабораторного образа допускается:

```bash
npm config set strict-ssl false
```

Эта настройка снижает безопасность TLS. В домашней среде и production лучше установить корпоративный корневой сертификат и прописать `npm config set cafile <path>`. В интерактивном курсе этот шаг должен быть автоматизирован в профиле "вузовская сеть" и явно отображаться в продвинутом режиме.
...
### 22.1 Первый JS MVP
- Экран: macOS-like окно с левой библиотекой блоков, центральной сборкой, правым кодом и нижней консолью.
- Обучающий режим: блоки фиксированы, подсказки включены, внутренние файлы скрыты.
- Продвинутый режим: можно редактировать сгенерированный код, `.npmrc`, `package.json`, тесты и runner pipeline.
- Блоки: переменные, функции, массивы, DOM, события, async/fetch, обработка ошибок, тесты.
- Runner: сначала browser sandbox для чистого JS, затем Node runner для файловых проектов и npm-зависимостей.
- Проверка: код выполняется, вывод сравнивается с ожиданием, ошибки разбираются по типу (`ReferenceError`, `SyntaxError`, async rejection, DOM selector not found).
