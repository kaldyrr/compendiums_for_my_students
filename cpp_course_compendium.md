...
- Базовая учебная связка: Visual Studio 2026 Community или актуальная Visual Studio 2022, MSVC Build Tools 14.50+, C++20 как основной стандарт, C++23 точечно, C++26 только как обзор будущих изменений.
- Для переносимых проектов используйте CMake 4.x, Ninja, `vcpkg.json` или Conan profile; для новичков оставляйте маршрут Visual Studio + CMake + vcpkg.
- Перед записью уроков фиксируйте версии компилятора, CMake, Windows SDK и менеджера пакетов в `README.md`.
- В интерактивном курсе проверяйте не только вывод программы, но и предупреждения компилятора, форматирование, санитайзеры, тесты и структуру проекта.
...
- Windows 10/11.
- Visual Studio 2026 Community или Visual Studio 2022 с workload **Desktop development with C++**.
- Git.
- Желательно: CMake, Ninja, vcpkg, Windows Terminal.
...
## 46. Заготовка для интерактивного курса

- Модули: `cpp-basics`, `cpp-functions`, `cpp-memory-raii`, `cpp-oop`, `cpp-stl`, `cpp-cmake-testing`, `cpp-concurrency`.
- Автопроверка: компиляция с `/W4` или `-Wall -Wextra`, запуск unit-тестов, проверка stdout, санитайзеры для memory/lifetime задач.
- Проектная линия: консольный менеджер задач -> файловое хранение -> классы -> тесты -> CMake -> небольшая библиотека.
- Платные элементы: ревью архитектуры итогового проекта, разбор UB и утечек памяти, профилирование, индивидуальный план перехода к game-dev/system-dev.
