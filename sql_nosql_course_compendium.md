...
- Базовый стек для курса: PostgreSQL 18.x как основная SQL СУБД, MongoDB 8.2 для документной модели, Redis 8 для cache/streams/pub-sub.
- Для production-блоков добавьте миграции, Testcontainers, Row-Level Security, audit logging, backup restore drills и observability.
- В отдельный модуль вынесите vector search: `pgvector`, hybrid search, embeddings, RAG-хранилища и ограничения такого подхода.
- Интерактивный курс должен проверять SQL-запросы, планы выполнения, миграции, индексы, транзакции и выбор модели данных.
...
- Модули: `sql-select`, `sql-joins`, `sql-design`, `sql-indexes`, `sql-transactions`, `nosql-modeling`, `db-ops`.
- Автопроверка: SQL assertions, `EXPLAIN`-задания, миграции на тестовой БД, seed data, Mongo aggregation checks.
- Проектная линия: схема интернет-магазина -> индексы -> транзакции -> API queries -> Mongo projection -> cache layer.
- Платные элементы: ревью схемы БД, оптимизация медленного запроса, выбор СУБД под бизнес-сценарий.
